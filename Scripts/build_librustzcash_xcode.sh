...
echo "cargo +nightly -Z build-std build --manifest-path ${PODS_TARGET_SRCROOT}/Cargo.toml --target $ZCASH_ACTIVE_ARCHITECTURE --release"
...
if [ ! -f ${ZCASH_LIB_RUST_BUILD_PATH}/${ZCASH_ACTIVE_ARCHITECTURE}/release/${ZCASH_LIB_RUST_NAME} ]; then
    cargo +nightly -Z build-std build --manifest-path ${PODS_TARGET_SRCROOT}/Cargo.toml --target $ZCASH_ACTIVE_ARCHITECTURE --release
    persist_environment
fi
